...
mcp__codex__codex:
  ...
  prompt: |
    ...
    8. **Introduction Review** (read the first two paragraphs on their own):
    ...
    10. **Audit and Case-Study Evidence Review** (when applicable):
       ...
       - Is a central pipeline mechanism shown through a compact worked trace, with toy/synthetic evidence labeled below historical and general claims?
       - Do claim-bearing detectors have suitable positive and benign/randomized negative controls, calibration, and false-positive/base-rate reporting?
       - Are effective configuration semantics checked, and are intentional label conflicts mapped at first encounter?
       - Is a proposed checklist or trace functionally compared with the closest prior art?
...
- **Mechanism clarity does not relax evidence** — add a worked trace, but label toy evidence honestly; require detector controls, effective configuration checks, upfront label maps, and prior-art positioning
