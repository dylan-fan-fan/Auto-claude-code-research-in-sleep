...
**Pass 4: Keyword Consistency and First-Use Definitions** — apply the Banana Rule: do not rename defined technical terms just to avoid repetition. If Methods defines a group, variable, or technique name, Results, Discussion, tables, and captions must use the same term. Then read the Abstract and main text as separate scopes: expand every technical abbreviation before first use, including CLI, API, GPU, and LLM. Check captions independently and remove one-off abbreviations unless they materially improve clarity.
