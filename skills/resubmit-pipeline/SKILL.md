...
Identify the object under study and require either a citation or a plain statement that it is unpublished, internal, or reconstructed. Verify claimed artifacts through the reviewer-facing submission package and test the stated entry command from a clean directory. A single case cannot retain a validated-framework claim without independent validation. Compare one canonical qualified claim across title, Abstract, Introduction, results, and Conclusion. Classify material code separately as present, called, executed, and result-backed. For a central pipeline mechanism, check for a compact worked trace and label its evidence role; adding a non-result-bearing illustration or label legend can be text-fixable, but running a synthetic counterfactual is a new experiment. Check detector controls, effective configuration semantics, upfront mappings for intentional label conflicts, and functional positioning of new checklists against prior art. Missing calibration, execution, or comparative evidence is not text-fixable merely by adding hedges; mark it `unaddressable-under-constraints` and trigger the existing scope escalation.
...
7. **Audit/case-study evidence passes, when applicable** — the target is identified, artifacts are reviewer-visible, generality matches independent evidence, claim strength is consistent, no execution claim rests only on code presence, the central mechanism has a correctly labeled worked trace, detector claims respect their controls, effective configuration is reported, label conflicts are mapped, and new checklists are positioned against prior art.
...
- **A worked illustration is not a free experiment.** Text-only resubmit may add a clearly labeled non-result-bearing example or legend, but synthetic execution, detector calibration, and new comparisons require scope escalation.
